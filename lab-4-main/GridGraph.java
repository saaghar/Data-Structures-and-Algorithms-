...
public class GridGraph implements DirectedGraph<GridGraph.Coord> {
    ...
    private boolean passable(int x, int y) {
        return x >= 0 && y >= 0 && x < width && y < height && passableChars.indexOf(grid[y][x]) >= 0;
    }
    ...
    public String showGrid(List<Coord> path) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++)
                w.print(path.contains(new Coord(x, y)) ? '*' : grid[y][x]);
            w.println();
        }
    ...
    public String toString() {
        ...
        w.println(showGrid());
        w.println("Example random coordinates and edges:");
