...
public class NPuzzle implements DirectedGraph<String> {
    ...
    public String toString() {
        ...
        w.println("States are strings of " + (N*N) + " chars '" + tiles.charAt(1) + "'...'" + tiles.charAt(N*N) + "', including exactly one '" + tiles.charAt(0) + "', denoting the empty tile; every " + N + " chars are interspersed with '" + separator + "'.");
        w.println();
        w.println("The traditional goal state is: " + makeState(tiles.substring(1, N*N) + tiles.charAt(0)));
        w.println("Example random states and edges:");
        ...
        for (int i = 0; i < 10; i++) {
            ...
            w.println(example + " --> " + edges.stream().map(e -> e.to()).collect(Collectors.joining(", ")));
        }
